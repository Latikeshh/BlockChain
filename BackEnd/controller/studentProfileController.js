...
exports.getMyProfile = async (req, res) => {
  try {
    ...
    const [block, form, studentRecord] = await Promise.all([
      StudentProfile.findOne({ studentId }),
      StudentForm.findOne({ studentId }),
      Student.findById(studentId).select("verify name enroll isProfileLocked"),
    ]);

    // Prefer latest form data whenever present so semester updates are reflected.
    if (form) {
      const { status, rejectNote, rejectSections, ...plain } = form.toObject();
      const studentLocked = studentRecord
        ? studentRecord.isProfileLocked
        : status !== "rejected";
    ...
    if (block) {
      ...
      return res.json({
        ...
        isProfileLocked: studentRecord ? studentRecord.isProfileLocked : true,
        hasProfile: true,
        verify: studentRecord ? studentRecord.verify : false,
        name: studentRecord?.name,
        enroll: studentRecord?.enroll,
    ...
    return res.status(200).json({
      success: true,
      hasProfile: false,
      isProfileLocked: studentRecord ? studentRecord.isProfileLocked : false,
      verify: studentRecord ? studentRecord.verify : false,
      name: studentRecord?.name,
      enroll: studentRecord?.enroll,
    });
