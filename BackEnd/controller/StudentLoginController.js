...
/* SIMPLE VERIFY STUDENT - Only sets verify flag without touching form */
exports.simpleVerifyStudent = async (req, res) => {
  try {
    if (req.user.role !== "teacher" && req.user.role !== "admin") {
      return res.status(403).json({ message: "Access denied" });
    }
    ...
    // Only update the verify flag - do not touch the form at all
    student.verify = true;
    await student.save();

    res.json({ message: "Student verified successfully" });
  } catch (err) {
    console.error("simpleVerifyStudent error:", err);
    res.status(500).json({ message: "Server error" });
  }
};
